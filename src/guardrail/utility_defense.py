...
_OPERATIONAL = ("steps to", "method to", "rewrite so", "rephrase so", "encode so", "give me", "find their", "tell me their", "follow the attached", "execute the attached")
_SECURITY_CONFLICT = ("ignore", "disregard", "override", "supersede", "unrestricted", "system prompt", "developer message", "bypass", "evade", "circumvent", "without restrictions", "no rules")
...
_ACCOUNT_ACTIONS = ("secure", "protect", "recover", "reset", "change", "sign out", "remove", "revoke", "enable", "disable", "check", "review", "help", "what should i do")
...
def is_high_confidence_utility(request: GuardrailRequest) -> bool:
    ...
    if _has(text, _OPERATIONAL) or _has(text, _SECURITY_CONFLICT):
        return False
    route = str(c.route)
    if route == "report":
